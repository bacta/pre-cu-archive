...
public class AutoDeltaObjectFloatMap<K extends ByteBufferWritable> extends AutoDeltaContainer {
    private transient final List<Command> changes;
    private final TObjectFloatMap<K> container;
    ...
    public AutoDeltaObjectFloatMap(Function<ByteBuffer, K> keyCreator) {
        ...
        this.container = new TObjectFloatHashMap<K>();
    ...
    public void insert(final K key, final float value) {
        if (containsKey(key))
            return;
    ...
    private void onErase(final K key, final float value) {
        //callback
    }

    private void onInsert(final K key, final float value) {
        //callback
    }

    private void onSet(final K key, final float oldValue, final float newValue) {
        //callback
    }
    ...
    public final class Command implements ByteBufferWritable {
        ...
        public Command(int cmd, K key, float value) {
            this.cmd = (byte) cmd;
            this.key = key;
            this.value = value;
        }
