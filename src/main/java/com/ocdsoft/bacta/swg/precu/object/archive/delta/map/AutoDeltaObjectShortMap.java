...
public class AutoDeltaObjectShortMap<K extends ByteBufferWritable> extends AutoDeltaContainer {
    private transient final List<Command> changes;
    private final TObjectShortMap<K> container;
    ...
    public AutoDeltaObjectShortMap(Function<ByteBuffer, K> keyCreator) {
        ...
        this.container = new TObjectShortHashMap<K>();
    ...
    public void insert(final K key, final short value) {
        if (containsKey(key))
            return;
    ...
    private void onErase(final K key, final short value) {
        //callback
    }

    private void onInsert(final K key, final short value) {
        //callback
    }

    private void onSet(final K key, final short oldValue, final short newValue) {
        //callback
    }
    ...
    public final class Command implements ByteBufferWritable {
        ...
        public Command(int cmd, K key, short value) {
            this.cmd = (byte) cmd;
            this.key = key;
            this.value = value;
        }
