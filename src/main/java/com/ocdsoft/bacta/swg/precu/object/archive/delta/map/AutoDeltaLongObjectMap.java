...
public class AutoDeltaLongObjectMap<V extends ByteBufferWritable> extends AutoDeltaContainer {
    private transient final List<Command> changes;
    private final TLongObjectMap<V> container;
    private transient int baselineCommandCount;
    private final Function<ByteBuffer, V> valueCreator;

    public AutoDeltaLongObjectMap(Function<ByteBuffer, V> valueCreator) {
        this.changes = new ArrayList<>(5);
        this.container = new TLongObjectHashMap<V>();
    ...
    public void insert(final long key, final V value) {
        if (containsKey(key))
            return;
    ...
    public void set(final long key, final V value) {
        if (!containsKey(key)) {
            //Inserting...
            final Command command = new Command(Command.ADD, key, value);
            container.put(key, value);
    ...
    private void onErase(final long key, final V value) {
        //callback
    }

    private void onInsert(final long key, final V value) {
        //callback
    }

    private void onSet(final long key, final V oldValue, final V newValue) {
        //callback
    }
    ...
    public final class Command implements ByteBufferWritable {
        ...
        public Command(int cmd, long key, V value) {
            this.cmd = (byte) cmd;
            this.key = key;
            this.value = value;
        }
