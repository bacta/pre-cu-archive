...
public class AutoDeltaStringShortMap extends AutoDeltaContainer {
    private transient final List<Command> changes;
    private final TObjectShortMap<String> container;
    private transient int baselineCommandCount;

    public AutoDeltaStringShortMap() {
        this.changes = new ArrayList<>(5);
        this.container = new TObjectShortHashMap<String>();
    ...
    public TObjectShortMap<String> getMap() {
        return container;
    }

    public void insert(final String key, final short value) {
        if (containsKey(key))
            return;
    ...
    private void onErase(final String key, final short value) {
        //callback
    }

    private void onInsert(final String key, final short value) {
        //callback
    }

    private void onSet(final String key, final short oldValue, final short newValue) {
        //callback
    }
    ...
    public final class Command implements ByteBufferWritable {
        ...
        public Command(int cmd, String key, short value) {
            this.cmd = (byte) cmd;
            this.key = key;
            this.value = value;
        }
