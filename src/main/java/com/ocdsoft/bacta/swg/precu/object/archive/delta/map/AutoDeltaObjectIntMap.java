...
public class AutoDeltaObjectIntMap<K extends ByteBufferWritable> extends AutoDeltaContainer {
    private transient final List<Command> changes;
    private final TObjectIntMap<K> container;
    private transient int baselineCommandCount;
    private final Function<ByteBuffer, K> keyCreator;

    public AutoDeltaObjectIntMap(Function<ByteBuffer, K> keyCreator) {
        this.changes = new ArrayList<>(5);
        this.container = new TObjectIntHashMap<K>();
    ...
    public void insert(final K key, final int value) {
        if (containsKey(key))
            return;
    ...
    private void onErase(final K key, final int value) {
        //callback
    }

    private void onInsert(final K key, final int value) {
        //callback
    }

    private void onSet(final K key, final int oldValue, final int newValue) {
        //callback
    }
    ...
    public final class Command implements ByteBufferWritable {
        ...
        public Command(int cmd, K key, int value) {
            this.cmd = (byte) cmd;
            this.key = key;
            this.value = value;
        }
