...
public class AutoDeltaShortObjectMap<V extends ByteBufferWritable> extends AutoDeltaContainer {
    private transient final List<Command> changes;
    private final TShortObjectMap<V> container;
    ...
    public AutoDeltaShortObjectMap(Function<ByteBuffer, V> valueCreator) {
        ...
        this.container = new TShortObjectHashMap<V>();
    ...
    public void insert(final short key, final V value) {
        if (containsKey(key))
            return;
    ...
    public void set(final short key, final V value) {
        if (!containsKey(key)) {
            //Inserting...
            final Command command = new Command(Command.ADD, key, value);
            container.put(key, value);
    ...
    private void onErase(final short key, final V value) {
        //callback
    }

    private void onInsert(final short key, final V value) {
        //callback
    }

    private void onSet(final short key, final V oldValue, final V newValue) {
        //callback
    }
    ...
    public final class Command implements ByteBufferWritable {
        ...
        public Command(int cmd, short key, V value) {
            this.cmd = (byte) cmd;
            this.key = key;
            this.value = value;
        }
