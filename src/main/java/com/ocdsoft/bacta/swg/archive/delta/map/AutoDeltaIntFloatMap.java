...
public class AutoDeltaIntFloatMap extends AutoDeltaContainer {
    private transient final List<Command> changes;
    private final TIntFloatMap container;
    private transient int baselineCommandCount;

    public AutoDeltaIntFloatMap() {
        this.changes = new ArrayList<>(5);
        this.container = new TIntFloatHashMap();
    ...
    public void insert(final int key, final float value) {
        if (containsKey(key))
            return;
    ...
    private void onErase(final int key, final float value) {
        //callback
    }

    private void onInsert(final int key, final float value) {
        //callback
    }

    private void onSet(final int key, final float oldValue, final float newValue) {
        //callback
    }
    ...
    public static class Command implements ByteBufferWritable {
        ...
        public Command(int cmd, int key, float value) {
            this.cmd = (byte) cmd;
            this.key = key;
            this.value = value;
        }
