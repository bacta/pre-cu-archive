...
public class AutoDeltaShortFloatMap extends AutoDeltaContainer {
    private transient final List<Command> changes;
    private final TShortFloatMap container;
    private transient int baselineCommandCount;

    public AutoDeltaShortFloatMap() {
        this.changes = new ArrayList<>(5);
        this.container = new TShortFloatHashMap();
    ...
    public void insert(final short key, final float value) {
        if (containsKey(key))
            return;
    ...
    public void set(final short key, final float value) {
        if (!containsKey(key)) {
            //Inserting...
            final Command command = new Command(Command.ADD, key, value);
            container.put(key, value);
    ...
    private void onErase(final short key, final float value) {
        //callback
    }

    private void onInsert(final short key, final float value) {
        //callback
    }

    private void onSet(final short key, final float oldValue, final float newValue) {
        //callback
    }
    ...
    public static class Command implements ByteBufferWritable {
        ...
        public Command(int cmd, short key, float value) {
            this.cmd = (byte) cmd;
            this.key = key;
            this.value = value;
        }
