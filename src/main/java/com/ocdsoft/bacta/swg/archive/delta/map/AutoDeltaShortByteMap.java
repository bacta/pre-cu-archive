...
public class AutoDeltaShortByteMap extends AutoDeltaContainer {
    ...
    public void insert(final short key, final byte value) {
        if (containsKey(key))
            return;
    ...
    public void set(final short key, final byte value) {
        if (!containsKey(key)) {
            //Inserting...
            final Command command = new Command(Command.ADD, key, value);
            container.put(key, value);
    ...
    private void onErase(final short key, final byte value) {
        //callback
    }

    private void onInsert(final short key, final byte value) {
        //callback
    }

    private void onSet(final short key, final byte oldValue, final byte newValue) {
        //callback
    }
    ...
    public static class Command implements ByteBufferWritable {
        ...
        public Command(int cmd, short key, byte value) {
            this.cmd = (byte) cmd;
            this.key = key;
            this.value = value;
        }
