...
public class AutoDeltaBoolShortMap extends AutoDeltaContainer {
    ...
    public void erase(final boolean key) {
        final short value = container.get(key ? (byte) 1 : (byte) 0);
    ...
    public short get(final boolean key) {
    ...
    public void insert(final boolean key, final short value) {
        if (containsKey(key))
            return;
    ...
    public void set(final boolean key, final short value) {
        if (!containsKey(key)) {
            //Inserting...
            final Command command = new Command(Command.ADD, key, value);
            container.put(key ? (byte) 1 : (byte) 0, value);
            ...
            final short oldValue = container.get(key ? (byte) 1 : (byte) 0);
            container.put(key ? (byte) 1 : (byte) 0, value);
            touch();
    ...
    private void onErase(final boolean key, final short value) {
        //callback
    }

    private void onInsert(final boolean key, final short value) {
        //callback
    }

    private void onSet(final boolean key, final short oldValue, final short newValue) {
        //callback
    }
    ...
    public static class Command implements ByteBufferWritable {
        ...
        public Command(int cmd, boolean key, short value) {
            this.cmd = (byte) cmd;
            this.key = key;
            this.value = value;
        }
