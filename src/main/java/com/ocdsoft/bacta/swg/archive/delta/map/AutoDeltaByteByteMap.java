...
public class AutoDeltaByteByteMap extends AutoDeltaContainer {
    ...
    private void onErase(final byte key, final byte value) {
        //callback
    }

    private void onInsert(final byte key, final byte value) {
        //callback
    }

    private void onSet(final byte key, final byte oldValue, final byte newValue) {
        //callback
    }
    ...
    public static class Command implements ByteBufferWritable {
        ...
        public Command(int cmd, byte key, byte value) {
            this.cmd = (byte) cmd;
            this.key = key;
            this.value = value;
        }
