...
public class AutoDeltaIntLongMap extends AutoDeltaContainer {
    private transient final List<Command> changes;
    private final TIntLongMap container;
    private transient int baselineCommandCount;

    public AutoDeltaIntLongMap() {
        this.changes = new ArrayList<>(5);
        this.container = new TIntLongHashMap();
    ...
    public void insert(final int key, final long value) {
        if (containsKey(key))
            return;
    ...
    private void onErase(final int key, final long value) {
        //callback
    }

    private void onInsert(final int key, final long value) {
        //callback
    }

    private void onSet(final int key, final long oldValue, final long newValue) {
        //callback
    }
    ...
    public static class Command implements ByteBufferWritable {
        ...
        public Command(int cmd, int key, long value) {
            this.cmd = (byte) cmd;
            this.key = key;
            this.value = value;
        }
