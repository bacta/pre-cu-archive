 ...
 */
public class AutoDeltaVectorTemplateWriter {
    private static final Logger LOGGER = LoggerFactory.getLogger(AutoDeltaVectorTemplateWriter.class);
    private static final String archivePath = "pre-cu/src/main/java/com/ocdsoft/bacta/swg/precu/object/archive/delta/vector";
    private static final String resourcesPath = new File(AutoDeltaVectorTemplateWriter.class.getResource("/").getFile()).getAbsolutePath();

    private final VelocityEngine velocityEngine;
    private final Template template;

    public AutoDeltaVectorTemplateWriter() {
        ...
        template = velocityEngine.getTemplate("/template/AutoDeltaVector.vm");
    }

    public void writeTemplates() {
        try {
            Files.readAllLines(Paths.get(resourcesPath, "AutoDeltaVector.txt"))
                    .stream()
    ...
    private void writeTemplate(final AutoDeltaSetContext context) {
        try {
            ...
            velocityContext.put("trove", context.listType.charAt(0) == 'T');
            velocityContext.put("listType", context.listType);
            velocityContext.put("listImpl", context.listImpl);
            ...
            final String fileName = String.format("AutoDelta%sVector.java", context.valueName);
    ...
    public static void main(final String[] args) {
        final AutoDeltaVectorTemplateWriter writer = new AutoDeltaVectorTemplateWriter();
