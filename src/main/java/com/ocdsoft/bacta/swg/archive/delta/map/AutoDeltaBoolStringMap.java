...
public class AutoDeltaBoolStringMap extends AutoDeltaContainer {
    private transient final List<Command> changes;
    private final TByteObjectMap<String> container;
    ...
    public void erase(final boolean key) {
        final String value = container.get(key ? (byte) 1 : (byte) 0);
        if (value != null) {
    ...
    public TByteObjectIterator<String> iterator() {
    ...
    public String get(final boolean key) {
    ...
    public void insert(final boolean key, final String value) {
        if (containsKey(key))
            return;
    ...
    public void set(final boolean key, final String value) {
        if (!containsKey(key)) {
            //Inserting...
            final Command command = new Command(Command.ADD, key, value);
            container.put(key ? (byte) 1 : (byte) 0, value);
            ...
            final String oldValue = container.get(key ? (byte) 1 : (byte) 0);
            container.put(key ? (byte) 1 : (byte) 0, value);
            touch();
    ...
    private void onErase(final boolean key, final String value) {
        //callback
    }

    private void onInsert(final boolean key, final String value) {
        //callback
    }

    private void onSet(final boolean key, final String oldValue, final String newValue) {
        //callback
    }
    ...
    public static class Command implements ByteBufferWritable {
        ...
        public Command(int cmd, boolean key, String value) {
            this.cmd = (byte) cmd;
            this.key = key;
            this.value = value;
        }
