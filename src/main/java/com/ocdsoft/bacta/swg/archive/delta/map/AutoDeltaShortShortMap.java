...
public class AutoDeltaShortShortMap extends AutoDeltaContainer {
    private transient final List<Command> changes;
    private final TShortShortMap container;
    private transient int baselineCommandCount;

    public AutoDeltaShortShortMap() {
        this.changes = new ArrayList<>(5);
        this.container = new TShortShortHashMap();
    ...
    public void insert(final short key, final short value) {
        if (containsKey(key))
            return;
    ...
    public void set(final short key, final short value) {
        if (!containsKey(key)) {
            //Inserting...
            final Command command = new Command(Command.ADD, key, value);
            container.put(key, value);
    ...
    private void onErase(final short key, final short value) {
        //callback
    }

    private void onInsert(final short key, final short value) {
        //callback
    }

    private void onSet(final short key, final short oldValue, final short newValue) {
        //callback
    }
    ...
    public static class Command implements ByteBufferWritable {
        ...
        public Command(int cmd, short key, short value) {
            this.cmd = (byte) cmd;
            this.key = key;
            this.value = value;
        }
