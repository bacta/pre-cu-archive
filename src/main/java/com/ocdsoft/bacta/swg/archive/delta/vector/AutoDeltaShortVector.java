...
public class AutoDeltaShortVector extends AutoDeltaContainer {
    private transient final List<Command> commands;
    private final TShortList v;
    ...
    public AutoDeltaShortVector(final int initialSize) {
        this.commands = new ArrayList<>(5);
        this.v = new TShortArrayList(initialSize);
        this.baselineCommandCount = 0;
    }
    ...
    public void set(final int element, final short value) {
    ...
    public int find(final short value) {
        for (int i = 0, size = v.size(); i < size; ++i) {
            if ((v.get(i)) == value)
                return i;
        }
    ...
    public void packDelta(final ByteBuffer buffer) {
        ...
        for (int i = 0, size = commands.size(); i < size; ++i) {
            ...
            switch (command.cmd) {
                ...
                case Command.SET: {
                    buffer.putShort(command.index);
                    final short value = command.value; //value will never be null in this case - ignore warning.
                    BufferUtil.put(buffer, value);
                    break;
                }
                case Command.SETALL: {
                    ...
                    for (int j = 0; j < command.index; ++j) {
                        ...
                        final short value = nextCommand.value;//value will never be null in this case - ignore warning.
                        BufferUtil.put(buffer, value);
                    }
                    break;
                }
    ...
    public void unpackDelta(final ByteBuffer buffer) {
        ...
        for (; i < commandCount; ++i) {
            ...
            switch (cmd) {
                ...
                case Command.SETALL: {
                    final short index = buffer.getShort();
                    final TShortList tempList = new TShortArrayList(index);
    ...
    private void onErase(final int index, final short oldValue) {
    }

    private void onInsert(final int index, final short value) {
    }

    private void onSet(final int index, final short oldValue, final short newValue) {
    }
    ...
    public final class Command {
        ...
        public Command(final byte cmd, final short index, final short value) {
            this.cmd = cmd;
            this.index = index;
            this.value = value;
        }
