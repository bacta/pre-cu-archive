package com.ocdsoft.bacta.swg.archive.delta.vector;
...
public class AutoDeltaFloatVector extends AutoDeltaContainer {
    private transient final List<Command> commands;
    private final TFloatList v;
    private transient int baselineCommandCount;

    public AutoDeltaFloatVector() {
        this.commands = new ArrayList<>(5);
        this.v = new TFloatArrayList();
        this.baselineCommandCount = 0;
    }

    public AutoDeltaFloatVector(final int initialSize) {
        this.commands = new ArrayList<>(5);
        this.v = new TFloatArrayList(initialSize);
        this.baselineCommandCount = 0;
    }
    ...
    public void erase(final int element) {
        if (element < size()) {
            ...
            commands.add(command);
            ++baselineCommandCount;
            final float oldValue = v.get(element);
            v.removeAt(element);
            touch();
    ...
    public void set(final int element, final float value) {
    ...
    public int find(final float value) {
        for (int i = 0, size = v.size(); i < size; ++i) {
            if ((v.get(i)) == value)
                return i;
        }
    ...
    public void packDelta(final ByteBuffer buffer) {
        ...
        for (int i = 0, size = commands.size(); i < size; ++i) {
            ...
            switch (command.cmd) {
                ...
                case Command.SET: {
                    buffer.putShort(command.index);
                    final float value = command.value; //value will never be null in this case - ignore warning.
                    BufferUtil.put(buffer, value);
                    break;
                }
                case Command.SETALL: {
                    ...
                    for (int j = 0; j < command.index; ++j) {
                        ...
                        final float value = nextCommand.value;//value will never be null in this case - ignore warning.
                        BufferUtil.put(buffer, value);
                    }
                    break;
                }
    ...
    public void unpackDelta(final ByteBuffer buffer) {
        ...
        for (; i < commandCount; ++i) {
            ...
            switch (cmd) {
                ...
                case Command.SETALL: {
                    final short index = buffer.getShort();
                    final TFloatList tempList = new TFloatArrayList(index);
    ...
    private void onErase(final int index, final float oldValue) {
    }

    private void onInsert(final int index, final float value) {
    }

    private void onSet(final int index, final float oldValue, final float newValue) {
    }
    ...
    public final class Command {
        ...
        public Command(final byte cmd, final short index, final float value) {
            this.cmd = cmd;
            this.index = index;
            this.value = value;
        }
