...
public class AutoDeltaLongShortMap extends AutoDeltaContainer {
    private transient final List<Command> changes;
    private final TLongShortMap container;
    private transient int baselineCommandCount;

    public AutoDeltaLongShortMap() {
        this.changes = new ArrayList<>(5);
        this.container = new TLongShortHashMap();
    ...
    public void insert(final long key, final short value) {
        if (containsKey(key))
            return;
    ...
    private void onErase(final long key, final short value) {
        //callback
    }

    private void onInsert(final long key, final short value) {
        //callback
    }

    private void onSet(final long key, final short oldValue, final short newValue) {
        //callback
    }
    ...
    public static class Command implements ByteBufferWritable {
        ...
        public Command(int cmd, long key, short value) {
            this.cmd = (byte) cmd;
            this.key = key;
            this.value = value;
        }
