...
public class AutoDeltaByteBoolMap extends AutoDeltaContainer {
    ...
    public void insert(final byte key, final boolean value) {
        if (containsKey(key))
            return;
    ...
    public void set(final byte key, final boolean value) {
        if (!containsKey(key)) {
            //Inserting...
            final Command command = new Command(Command.ADD, key, value);
            container.put(key, value ? (byte) 1 : (byte) 0);
    ...
    private void onErase(final byte key, final boolean value) {
        //callback
    }

    private void onInsert(final byte key, final boolean value) {
        //callback
    }

    private void onSet(final byte key, final boolean oldValue, final boolean newValue) {
        //callback
    }
    ...
    public static class Command implements ByteBufferWritable {
        ...
        public Command(int cmd, byte key, boolean value) {
            this.cmd = (byte) cmd;
            this.key = key;
            this.value = value;
        }
