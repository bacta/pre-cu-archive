...
public class AutoDeltaFloatLongMap extends AutoDeltaContainer {
    private transient final List<Command> changes;
    private final TFloatLongMap container;
    private transient int baselineCommandCount;

    public AutoDeltaFloatLongMap() {
        this.changes = new ArrayList<>(5);
        this.container = new TFloatLongHashMap();
    ...
    public void insert(final float key, final long value) {
        if (containsKey(key))
            return;
    ...
    public void set(final float key, final long value) {
        if (!containsKey(key)) {
            //Inserting...
            final Command command = new Command(Command.ADD, key, value);
            container.put(key, value);
    ...
    private void onErase(final float key, final long value) {
        //callback
    }

    private void onInsert(final float key, final long value) {
        //callback
    }

    private void onSet(final float key, final long oldValue, final long newValue) {
        //callback
    }
    ...
    public static class Command implements ByteBufferWritable {
        ...
        public Command(int cmd, float key, long value) {
            this.cmd = (byte) cmd;
            this.key = key;
            this.value = value;
        }
