...
public class AutoDeltaIntIntMap extends AutoDeltaContainer {
    private transient final List<Command> changes;
    private final TIntIntMap container;
    private transient int baselineCommandCount;

    public AutoDeltaIntIntMap() {
        this.changes = new ArrayList<>(5);
        this.container = new TIntIntHashMap();
    ...
    public void insert(final int key, final int value) {
        if (containsKey(key))
            return;
    ...
    private void onErase(final int key, final int value) {
        //callback
    }

    private void onInsert(final int key, final int value) {
        //callback
    }

    private void onSet(final int key, final int oldValue, final int newValue) {
        //callback
    }
    ...
    public static class Command implements ByteBufferWritable {
        ...
        public Command(int cmd, int key, int value) {
            this.cmd = (byte) cmd;
            this.key = key;
            this.value = value;
        }
