...
public class AutoDeltaFloatByteMap extends AutoDeltaContainer {
    ...
    public void insert(final float key, final byte value) {
        if (containsKey(key))
            return;
    ...
    public void set(final float key, final byte value) {
        if (!containsKey(key)) {
            //Inserting...
            final Command command = new Command(Command.ADD, key, value);
            container.put(key, value);
    ...
    private void onErase(final float key, final byte value) {
        //callback
    }

    private void onInsert(final float key, final byte value) {
        //callback
    }

    private void onSet(final float key, final byte oldValue, final byte newValue) {
        //callback
    }
    ...
    public static class Command implements ByteBufferWritable {
        ...
        public Command(int cmd, float key, byte value) {
            this.cmd = (byte) cmd;
            this.key = key;
            this.value = value;
        }
