...
public class AutoDeltaByteIntMap extends AutoDeltaContainer {
    ...
    public void insert(final byte key, final int value) {
        if (containsKey(key))
            return;
    ...
    public void set(final byte key, final int value) {
        if (!containsKey(key)) {
            //Inserting...
            final Command command = new Command(Command.ADD, key, value);
            container.put(key, value);
    ...
    private void onErase(final byte key, final int value) {
        //callback
    }

    private void onInsert(final byte key, final int value) {
        //callback
    }

    private void onSet(final byte key, final int oldValue, final int newValue) {
        //callback
    }
    ...
    public static class Command implements ByteBufferWritable {
        ...
        public Command(int cmd, byte key, int value) {
            this.cmd = (byte) cmd;
            this.key = key;
            this.value = value;
        }
