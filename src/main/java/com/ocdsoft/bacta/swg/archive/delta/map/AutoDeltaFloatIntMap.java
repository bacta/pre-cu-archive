...
public class AutoDeltaFloatIntMap extends AutoDeltaContainer {
    private transient final List<Command> changes;
    private final TFloatIntMap container;
    ...
    public void insert(final float key, final int value) {
        if (containsKey(key))
            return;
    ...
    public void set(final float key, final int value) {
        if (!containsKey(key)) {
            //Inserting...
            final Command command = new Command(Command.ADD, key, value);
            container.put(key, value);
    ...
    private void onErase(final float key, final int value) {
        //callback
    }

    private void onInsert(final float key, final int value) {
        //callback
    }

    private void onSet(final float key, final int oldValue, final int newValue) {
        //callback
    }
    ...
    public static class Command implements ByteBufferWritable {
        ...
        public Command(int cmd, float key, int value) {
            this.cmd = (byte) cmd;
            this.key = key;
            this.value = value;
        }
