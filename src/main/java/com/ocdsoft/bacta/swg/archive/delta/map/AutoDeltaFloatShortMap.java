...
public class AutoDeltaFloatShortMap extends AutoDeltaContainer {
    private transient final List<Command> changes;
    private final TFloatShortMap container;
    private transient int baselineCommandCount;

    public AutoDeltaFloatShortMap() {
        this.changes = new ArrayList<>(5);
        this.container = new TFloatShortHashMap();
    ...
    public void insert(final float key, final short value) {
        if (containsKey(key))
            return;
    ...
    public void set(final float key, final short value) {
        if (!containsKey(key)) {
            //Inserting...
            final Command command = new Command(Command.ADD, key, value);
            container.put(key, value);
    ...
    private void onErase(final float key, final short value) {
        //callback
    }

    private void onInsert(final float key, final short value) {
        //callback
    }

    private void onSet(final float key, final short oldValue, final short newValue) {
        //callback
    }
    ...
    public static class Command implements ByteBufferWritable {
        ...
        public Command(int cmd, float key, short value) {
            this.cmd = (byte) cmd;
            this.key = key;
            this.value = value;
        }
