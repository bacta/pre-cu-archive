...
public class AutoDeltaLongFloatMap extends AutoDeltaContainer {
    private transient final List<Command> changes;
    private final TLongFloatMap container;
    private transient int baselineCommandCount;

    public AutoDeltaLongFloatMap() {
        this.changes = new ArrayList<>(5);
        this.container = new TLongFloatHashMap();
    ...
    public void insert(final long key, final float value) {
        if (containsKey(key))
            return;
    ...
    private void onErase(final long key, final float value) {
        //callback
    }

    private void onInsert(final long key, final float value) {
        //callback
    }

    private void onSet(final long key, final float oldValue, final float newValue) {
        //callback
    }
    ...
    public static class Command implements ByteBufferWritable {
        ...
        public Command(int cmd, long key, float value) {
            this.cmd = (byte) cmd;
            this.key = key;
            this.value = value;
        }
