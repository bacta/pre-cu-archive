...
public class AutoDeltaLongLongMap extends AutoDeltaContainer {
    private transient final List<Command> changes;
    private final TLongLongMap container;
    ...
    private void onErase(final long key, final long value) {
        //callback
    }

    private void onInsert(final long key, final long value) {
        //callback
    }

    private void onSet(final long key, final long oldValue, final long newValue) {
        //callback
    }
    ...
    public static class Command implements ByteBufferWritable {
        ...
        public Command(int cmd, long key, long value) {
            this.cmd = (byte) cmd;
            this.key = key;
            this.value = value;
        }
