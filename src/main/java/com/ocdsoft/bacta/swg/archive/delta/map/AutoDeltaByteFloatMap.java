...
public class AutoDeltaByteFloatMap extends AutoDeltaContainer {
    ...
    public void insert(final byte key, final float value) {
        if (containsKey(key))
            return;
    ...
    public void set(final byte key, final float value) {
        if (!containsKey(key)) {
            //Inserting...
            final Command command = new Command(Command.ADD, key, value);
            container.put(key, value);
    ...
    private void onErase(final byte key, final float value) {
        //callback
    }

    private void onInsert(final byte key, final float value) {
        //callback
    }

    private void onSet(final byte key, final float oldValue, final float newValue) {
        //callback
    }
    ...
    public static class Command implements ByteBufferWritable {
        ...
        public Command(int cmd, byte key, float value) {
            this.cmd = (byte) cmd;
            this.key = key;
            this.value = value;
        }
