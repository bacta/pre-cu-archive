...
public class AutoDeltaFloatBoolMap extends AutoDeltaContainer {
    private transient final List<Command> changes;
    private final TFloatByteMap container;
    private transient int baselineCommandCount;

    public AutoDeltaFloatBoolMap() {
        this.changes = new ArrayList<>(5);
        this.container = new TFloatByteHashMap();
    ...
    public void insert(final float key, final boolean value) {
        if (containsKey(key))
            return;
    ...
    public void set(final float key, final boolean value) {
        if (!containsKey(key)) {
            //Inserting...
            final Command command = new Command(Command.ADD, key, value);
            container.put(key, value ? (byte) 1 : (byte) 0);
    ...
    private void onErase(final float key, final boolean value) {
        //callback
    }

    private void onInsert(final float key, final boolean value) {
        //callback
    }

    private void onSet(final float key, final boolean oldValue, final boolean newValue) {
        //callback
    }
    ...
    public static class Command implements ByteBufferWritable {
        ...
        public Command(int cmd, float key, boolean value) {
            this.cmd = (byte) cmd;
            this.key = key;
            this.value = value;
        }
