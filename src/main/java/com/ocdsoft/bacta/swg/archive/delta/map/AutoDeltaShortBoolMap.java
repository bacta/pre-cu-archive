...
public class AutoDeltaShortBoolMap extends AutoDeltaContainer {
    private transient final List<Command> changes;
    private final TShortByteMap container;
    private transient int baselineCommandCount;

    public AutoDeltaShortBoolMap() {
        this.changes = new ArrayList<>(5);
        this.container = new TShortByteHashMap();
    ...
    public void insert(final short key, final boolean value) {
        if (containsKey(key))
            return;
    ...
    public void set(final short key, final boolean value) {
        if (!containsKey(key)) {
            //Inserting...
            final Command command = new Command(Command.ADD, key, value);
            container.put(key, value ? (byte) 1 : (byte) 0);
    ...
    private void onErase(final short key, final boolean value) {
        //callback
    }

    private void onInsert(final short key, final boolean value) {
        //callback
    }

    private void onSet(final short key, final boolean oldValue, final boolean newValue) {
        //callback
    }
    ...
    public static class Command implements ByteBufferWritable {
        ...
        public Command(int cmd, short key, boolean value) {
            this.cmd = (byte) cmd;
            this.key = key;
            this.value = value;
        }
