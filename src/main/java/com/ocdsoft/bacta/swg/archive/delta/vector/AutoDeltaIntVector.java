...
public class AutoDeltaIntVector extends AutoDeltaContainer {
    private transient final List<Command> commands;
    private final TIntList v;
    private transient int baselineCommandCount;

    public AutoDeltaIntVector() {
        this.commands = new ArrayList<>(5);
        this.v = new TIntArrayList();
        this.baselineCommandCount = 0;
    }

    public AutoDeltaIntVector(final int initialSize) {
        this.commands = new ArrayList<>(5);
        this.v = new TIntArrayList(initialSize);
        this.baselineCommandCount = 0;
    }
    ...
    public void set(final int element, final int value) {
    ...
    public void set(final TIntList value) {
    ...
    public void packDelta(final ByteBuffer buffer) {
        ...
        for (int i = 0, size = commands.size(); i < size; ++i) {
            ...
            switch (command.cmd) {
                ...
                case Command.SET: {
                    buffer.putShort(command.index);
                    final int value = command.value; //value will never be null in this case - ignore warning.
                    BufferUtil.put(buffer, value);
                    break;
                }
                case Command.SETALL: {
                    ...
                    for (int j = 0; j < command.index; ++j) {
                        ...
                        final int value = nextCommand.value;//value will never be null in this case - ignore warning.
                        BufferUtil.put(buffer, value);
                    }
                    break;
                }
    ...
    public void unpackDelta(final ByteBuffer buffer) {
        ...
        for (; i < commandCount; ++i) {
            ...
            switch (cmd) {
                ...
                case Command.SETALL: {
                    final short index = buffer.getShort();
                    final TIntList tempList = new TIntArrayList(index);
    ...
    private void onErase(final int index, final int oldValue) {
    }

    private void onInsert(final int index, final int value) {
    }

    private void onSet(final int index, final int oldValue, final int newValue) {
    }
    ...
    public final class Command {
        ...
        public Command(final byte cmd, final short index, final int value) {
            this.cmd = cmd;
            this.index = index;
            this.value = value;
        }
