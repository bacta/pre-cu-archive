...
public class AutoDeltaLongByteMap extends AutoDeltaContainer {
    ...
    public void insert(final long key, final byte value) {
        if (containsKey(key))
            return;
    ...
    private void onErase(final long key, final byte value) {
        //callback
    }

    private void onInsert(final long key, final byte value) {
        //callback
    }

    private void onSet(final long key, final byte oldValue, final byte newValue) {
        //callback
    }
    ...
    public static class Command implements ByteBufferWritable {
        ...
        public Command(int cmd, long key, byte value) {
            this.cmd = (byte) cmd;
            this.key = key;
            this.value = value;
        }
