...
public class AutoDeltaStringVector extends AutoDeltaContainer {
    private transient final List<Command> commands;
    private final List<String> v;
    private transient int baselineCommandCount;

    public AutoDeltaStringVector() {
        this.commands = new ArrayList<>(5);
        this.v = new ArrayList<String>();
        this.baselineCommandCount = 0;
    }

    public AutoDeltaStringVector(final int initialSize) {
        this.commands = new ArrayList<>(5);
        this.v = new ArrayList<String>(initialSize);
        this.baselineCommandCount = 0;
    }
    ...
    public void erase(final int element) {
        if (element < size()) {
            ...
            commands.add(command);
            ++baselineCommandCount;
            final String oldValue = v.get(element);
            v.remove(element);
            touch();
    ...
    public void set(final int element, final String value) {
        ...
        if (element >= v.size()) {
            for (int i = 0, size = v.size(); i < element + 1 - size; ++i)
                v.add(null); //Should this be empty string instead?
        }
    ...
    public void set(final List<String> value) {
    ...
    public void packDelta(final ByteBuffer buffer) {
        ...
        for (int i = 0, size = commands.size(); i < size; ++i) {
            ...
            switch (command.cmd) {
                ...
                case Command.SET: {
                    buffer.putShort(command.index);
                    final String value = command.value; //value will never be null in this case - ignore warning.
                    BufferUtil.put(buffer, value);
                    break;
                }
                case Command.SETALL: {
                    ...
                    for (int j = 0; j < command.index; ++j) {
                        ...
                        final String value = nextCommand.value;//value will never be null in this case - ignore warning.
                        BufferUtil.put(buffer, value);
                    }
                    break;
                }
    ...
    public void unpackDelta(final ByteBuffer buffer) {
        ...
        for (; i < commandCount; ++i) {
            ...
            switch (cmd) {
                ...
                case Command.SETALL: {
                    final short index = buffer.getShort();
                    final List<String> tempList = new ArrayList<String>(index);
    ...
    private void onErase(final int index, final String oldValue) {
    }

    private void onInsert(final int index, final String value) {
    }

    private void onSet(final int index, final String oldValue, final String newValue) {
    }
    ...
    public final class Command {
        ...
        public Command(final byte cmd, final short index, final String value) {
            this.cmd = cmd;
            this.index = index;
            this.value = value;
        }
