...
public class AutoDeltaBoolByteMap extends AutoDeltaContainer {
    ...
    public void erase(final boolean key) {
        final byte value = container.get(key ? (byte) 1 : (byte) 0);
    ...
    public byte get(final boolean key) {
    ...
    public void insert(final boolean key, final byte value) {
        if (containsKey(key))
            return;
    ...
    public void set(final boolean key, final byte value) {
        if (!containsKey(key)) {
            //Inserting...
            final Command command = new Command(Command.ADD, key, value);
            container.put(key ? (byte) 1 : (byte) 0, value);
            ...
            final byte oldValue = container.get(key ? (byte) 1 : (byte) 0);
            container.put(key ? (byte) 1 : (byte) 0, value);
            touch();
    ...
    private void onErase(final boolean key, final byte value) {
        //callback
    }

    private void onInsert(final boolean key, final byte value) {
        //callback
    }

    private void onSet(final boolean key, final byte oldValue, final byte newValue) {
        //callback
    }
    ...
    public static class Command implements ByteBufferWritable {
        ...
        public Command(int cmd, boolean key, byte value) {
            this.cmd = (byte) cmd;
            this.key = key;
            this.value = value;
        }
