...
public class AutoDeltaBoolBoolMap extends AutoDeltaContainer {
    private transient final List<Command> changes;
    private final TByteByteMap container;
    ...
    public void erase(final boolean key) {
        final boolean value = container.get(key ? (byte) 1 : (byte) 0) != 0;
    ...
    public boolean get(final boolean key) {
        return container.get(key ? (byte) 1 : (byte) 0) != 0;
    }
    ...
    public void insert(final boolean key, final boolean value) {
        if (containsKey(key))
            return;

        final Command command = new Command(Command.ADD, key, value);
        container.put(key ? (byte) 1 : (byte) 0, value ? (byte) 1 : (byte) 0);
        touch();
        onInsert(key, value);
        changes.add(command);
        ++baselineCommandCount;
    ...
    public void set(final boolean key, final boolean value) {
        if (!containsKey(key)) {
            //Inserting...
            final Command command = new Command(Command.ADD, key, value);
            container.put(key ? (byte) 1 : (byte) 0, value ? (byte) 1 : (byte) 0);
            ...
            final boolean oldValue = container.get(key ? (byte) 1 : (byte) 0) != 0;
            container.put(key ? (byte) 1 : (byte) 0, value ? (byte) 1 : (byte) 0);
            touch();
    ...
    public void unpack(final ByteBuffer buffer) {
        ...
        for (int i = 0; i < commandCount; ++i) {
            ...
            container.put(command.key ? (byte) 1 : (byte) 0, command.value ? (byte) 1 : (byte) 0);
    ...
    private void onErase(final boolean key, final boolean value) {
        //callback
    }

    private void onInsert(final boolean key, final boolean value) {
        //callback
    }

    private void onSet(final boolean key, final boolean oldValue, final boolean newValue) {
        //callback
    }
    ...
    public static class Command implements ByteBufferWritable {
        ...
        public Command(int cmd, boolean key, boolean value) {
            this.cmd = (byte) cmd;
            this.key = key;
            this.value = value;
        }
