...
public class AutoDeltaBoolIntMap extends AutoDeltaContainer {
    private transient final List<Command> changes;
    private final TByteIntMap container;
    ...
    public void erase(final boolean key) {
        final int value = container.get(key ? (byte) 1 : (byte) 0);
    ...
    public int get(final boolean key) {
    ...
    public void insert(final boolean key, final int value) {
        if (containsKey(key))
            return;
    ...
    public void set(final boolean key, final int value) {
        if (!containsKey(key)) {
            //Inserting...
            final Command command = new Command(Command.ADD, key, value);
            container.put(key ? (byte) 1 : (byte) 0, value);
            ...
            final int oldValue = container.get(key ? (byte) 1 : (byte) 0);
            container.put(key ? (byte) 1 : (byte) 0, value);
            touch();
    ...
    private void onErase(final boolean key, final int value) {
        //callback
    }

    private void onInsert(final boolean key, final int value) {
        //callback
    }

    private void onSet(final boolean key, final int oldValue, final int newValue) {
        //callback
    }
    ...
    public static class Command implements ByteBufferWritable {
        ...
        public Command(int cmd, boolean key, int value) {
            this.cmd = (byte) cmd;
            this.key = key;
            this.value = value;
        }
